...
int main() {
	...
	Text whiteScoreText, blackScoreText;
	whiteScoreText.setFont(font);
	whiteScoreText.setCharacterSize(24);
	whiteScoreText.setFillColor(Color::Red); // or Black depending on board color
	whiteScoreText.setPosition(50, 10);

	blackScoreText.setFont(font);
	blackScoreText.setCharacterSize(24);
	blackScoreText.setFillColor(Color::Red);
	blackScoreText.setPosition(700, 10); // adjust for your window size
	...
	while (window.isOpen()) {
			...
			if (event.type == Event::MouseButtonReleased && event.mouseButton.button == Mouse::Left && isDragging) {
				...
				if (valid) {
					...
					if (board[to.y][to.x] != 0) {
						...
						if (captured > 0) blackScore += getPieceValue(captured); // black captured white
						if (captured < 0) whiteScore += getPieceValue(captured); // white captured black
					...
					// --- PAWN PROMOTION ---
					int absPiece = abs(piece);
					if (absPiece == 1) { // it's a pawn
						if ((piece > 0 && to.y == 7) || (piece < 0 && to.y == 0)) {
							// Promote to queen
							int promotedPiece = piece > 0 ? 5 : -5; // 5 = queen
							board[to.y][to.x] = promotedPiece;

							// Update the sprite
							if (piece > 0) {
								whitePawns[dragIndex].setTexture(whitequeenTexture);
							}
							else {
								blackpawn[dragIndex].setTexture(blackqueenTexture);
							}
						}
					}
		...
		for (int row = 0; row < 8; row++) {
			for (int col = 0; col < 8; col++) {
				...
				if ((row + col) % 2 == 0)
					...
					Rectangle.setFillColor(Color(100, 149, 237));
		...
		whiteScoreText.setString("White: " + to_string(whiteScore));
		blackScoreText.setString("Black: " + to_string(blackScore));
